...
app.get('/api/drivers', function(req, res) {
	req.models.driver.all(function(err, driverRS) {
		if (err)
			...
			res.json(driverRS.map(function(c) {
				return c.serialize();
			}));
		}
	});
...
if (enableSimulator) {
	simulator.simulate_gps(io);
	simulator.simulate_distance(io);
	simulator.simulate_fuel(io);
	simulator.simulate_face(io);
}
